...
class MemoryManager:
    ...
    负责管理单个会话的结构化业务状态，同时把运行事件写入 transcript store。
    状态更新由确定性 reducer 和业务规则共同完成，不把最终写权限交给模型。
    """
    ...
    def enrich_request(self, request: ChatRequest, state: SessionStateSnapshot) -> ChatRequest:
        """将会话状态注入到请求 metadata 中，供路由器和工具规划器使用。"""
    ...
    def update(
        ...
        更新来源包括路由结果、工具结果、待确认动作、停止原因和最终回答。
        RuntimeStateReducer 负责流程阶段类状态，当前方法负责槽位、工具缓存、
        最近轮次窗口和简短摘要等短期业务记忆。
        """
        reduced = state
